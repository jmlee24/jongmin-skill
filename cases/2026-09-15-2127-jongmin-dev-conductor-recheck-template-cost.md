...
**관측 추가 (2026-09-14~15 다음 웨이브, 같은 프로젝트)**: 재확인 SendMessage 2회가 **5분·8분**으로 정상
범위 — 591k 재발 없음. 단 이 웨이브는 fix 레인 범위가 작았고(F1 20m·F2 34m) 템플릿은 그대로였으므로,
비용 정상화가 범위 효과인지 절차 효과인지 **미분리**. 아울러 T1이 이 절차를 "신설 제안"으로 오인해 Codex가
기존 절차로 재분류 — 발견성 케이스(procedure-discoverability) 참조. recheck 모드 제안은 유지하되 근거는
n=1(591k)이며 재현 실패 1회.
