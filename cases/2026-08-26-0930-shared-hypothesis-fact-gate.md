...
**관측 추가 (2026-08-29 YNprinting, P3·P4)**: 구현 산물(로더 14줄)이 설계 원칙(KI-34)으로 승격돼 정당한
설계안을 기각 — 근거가 순환("필드에 없으므로 무시"). 처방 병합: 규약·제약 인용 시 근거가 **설계 결정인지
구현 우연인지** 구분, 구현이 근거면 그 구현 변경 시 규약도 무효. 줄 번호 인용이 같은 세션 자기 리팩터로
부패(:808→:1024) → 인수인계에 실림 → 교차검증이 잡음.
