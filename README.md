...
완료까지 반복한다. 이중 종료 게이트(장부 완료 AND 독립 리뷰어 판정 — 실행자의 "완료했다"는
게이트 입력이 아님), 서킷 브레이커(변경 없는 3루프·동일 에러 3회 시 중단/접근 전환).

**쓸 때**: 완료 조건이 명확하고 검증 명령으로 표현 가능한 작업을 끝까지 밀어붙일 때.
...
후 질문 도구 자체가 차단된다(`disallowed-tools`). 판단 필요 지점은 보수적 기본값 선택 + 유예
장부 기록(선택지/선택/근거/되돌리는 법) 또는 갈래 보류 — 몰래 결정도, 정지도 하지 않는다.
