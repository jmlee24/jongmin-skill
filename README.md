...
3. **공용 부품** ([shared/](plugins/jongmin-skills/shared/)) — model-tiers(티어 정의) /
   codex-lane(Codex CLI 실행 패턴) / executor-prompt(쓰기 레인 5요소 템플릿) /
   landing-check(착지 검증 체크리스트) / ledger(진행·유예 장부 규격, `~/.claude/jongmin-ledgers/`)

## 스킬 목록

### [jongmin-dev-conductor](plugins/jongmin-skills/skills/jongmin-dev-conductor/SKILL.md) — 다중 레인 개발 편성

**수행**: 지휘부(메인 세션)는 구현을 직접 하지 않고 레인을 편성한다 — 쓰기 레인(T2 에이전트,
구현+테스트+커밋), 읽기 레인(CX, 명세·반박), 리뷰 레인(lane-reviewer, SHA 범위 독립 리뷰).
착지 검증(테스트 직접 재실행 + 결정론 게이트) → 리뷰 → 지적사항 접합 → 최종 보고까지 7단계.
...
**수행**: 완료 조건을 오라클(실행 가능한 검증 명령)로 확정한 뒤, 루프 1회 = 태스크 1개로
완료까지 반복한다. **플러그인 Stop 가드**(상태 파일 기반)가 미완료 상태의 자의적 턴 종료를
되민다 — 중간 질문도 frontmatter로 차단. 이중 종료 게이트(장부 완료 AND 독립 리뷰어 판정)
통과 후에만 exit_signal 기록·정지. 백스톱: no-progress 3회·최대 50회·데드라인(기본 4h).
스코프 고정 — 발견물은 기록만, 확장 금지(ralph의 스토리 추가와 반대). `cancel` 인자로 중단.
...
**수행**: 자리 비우기 전 걸어두는 무질문 자율 모드. 발진 계약(스코프·금지선·데드라인) 1회 확정
후 질문 도구 자체가 차단되고(`disallowed-tools`), loop와 같은 **Stop 가드**가 데드라인 전
자의적 정지를 되민다. 판단 필요 지점은 보수적 기본값 + 유예 장부 기록, **중대 판단(되돌리기
어려움·데이터 정합성)은 CX 반박 자문을 곁들인 공동 판단** — 몰래 결정도, 정지도 하지 않는다.
귀환 시 한 화면 보고(완료/유예 결정/보류·실패/다음 결정 지점).
...
**수행**: 계획 = 실행 프롬프트의 묶음. 모호성 인터뷰(3~5문항, 무인 시 가정 명시로 생략) →
근거 수집 → 단계마다 {실행 주체 라우팅(직접/executor/conductor/sortie/loop), 크기, 검증 오라클,
결정 지점}이 붙은 편성표 작성 → CX 반박 자문 → 채팅 제시. 각 단계는 executor-prompt 5요소로
즉시 컴파일 가능해야 계획 완성으로 친다. 계획만 산출하고 실행하지 않는다.
...
1. `plugins/jongmin-skills/skills/<스킬이름>/SKILL.md` 생성
   - frontmatter `name`(소문자·숫자·하이픈) + `description`(**발동 조건만** — 트리거 어휘 + 부정
     조건, 워크플로 요약 금지, "언급하면" 금지·"요청하면"으로)
   - 본문 500줄 이하, 모델은 티어명(T1/T2/CX)으로만, shared/ 부품은 참조(복붙 금지)
