---
name: skill-forge
description: jongmin-skills 패밀리의 스킬을 제작·평가·개선할 때 사용한다 — 슬래시 명령(/jongmin-skills:skill-forge)으로만 시작되며 자연어 요청으로는 발동하지 않는다 (disable-model-invocation). 스킬이 아닌 일반 코드·문서 작업에는 사용하지 않는다.
...
초안 규약:
- description은 **무엇+언제** (공식 규칙): 제3인칭으로 스킬이 하는 일 한 줄 + 슬래시 전용 명시 + 부정 조건
  (트리거 어휘 없음 — 자연어 발동이 없다). 워크플로 상세 요약 금지 — 모델이 본문 대신 요약만 따라가는 실패
...
- 모델은 티어명(T1/T2/CX)으로만. **패밀리 스킬은 슬래시 전용(`disable-model-invocation: true`)이 기본**이다 —
  이 플래그는 자연어 발동의 완전 차단이자(실측) 시스템 프롬프트 미적재(컨텍스트 0)다. 자연어 자동 탐지는
  구체적 실패를 예방하고 오발동 비용보다 이익이 큼을 근거로 기록한 예외에만 둔다. description에 슬래시 전용임을
  명시하고(본문 반복 금지 — README 호출 규약이 정본), 트리거 어휘 따옴표는 넣지 않는다. 질문 금지류 계약은
  `disallowed-tools`로 하드 강제하되, **차단이 같은 턴 후속 스킬에 전파됨**(실측)을 본문에 경고한다.
...
### eval — 경계 테스트 (슬래시 전용 전제)

자연어 발동이 없으므로 "발동 여부"가 아니라 **호출된 뒤의 경계 판정**을 본다. 범위 안 요청 3개 + 범위 밖
요청 3개(다른 스킬 몫·개발/비개발 경계·금지선)를 구성한다:

1. **데스크 판정 (기본)** — 각 요청을 본문의 경계·실패 정책과 대조해 "진행 / 중단·타 스킬 안내"를 추론
   판정. 비용 0, 모든 eval에서 수행. `validate.mjs` ④가 frontmatter 플래그·description 계약을 기계 검사한다.
2. **실 프로브 (릴리스 게이트 한정)** — `claude -p "/jongmin-skills:<name> <범위 밖 요청>" --output-format
   stream-json --max-turns 3`로 본문 로드 후 중단·안내가 실제로 나오는지 확인. 회당 약 $0.3 — 신규 스킬
   릴리스나 경계 변경 시에만.

경계 오판은 차단이 아니라 **본문 경계 문구 개선 + 사례 축적**으로 대응한다. shared/ 문서 변경 시 참조하는
전 스킬의 데스크 eval을 재실행한다.
...
- 자연어 자동 발동 기본 폐기 → 전 스킬 슬래시 전용 (v1.4, 2026-09-10) — 단독 운영자가 스킬명을 알고 있고,
  상시 적재 description 5건(약 1.5KB/세션)의 이득이 없었다. 반론이던 handoff 컴팩션 안전망은 클코 자체
  컨텍스트 요약으로 대체
