...
파일은 주 소비자가 **미래 세션·기계**일 때(컴팩션 임박·계정 전환·기기 이동·중단),
...
- [ ] 진행 장부 위치와 최종 상태 (진행/유예/실패 항목) — **참조만, 내용 복제 금지**
      (정본은 진행 장부 1개 — [ledger.md](../../shared/ledger.md) 정본 단일화)
- [ ] 활성 레인: 백그라운드 잡·에이전트·codex 스레드 ID와 각각의 상태
- [ ] 다음 단계: 재개 시 제일 먼저 할 일 1~3개 (명령 수준으로 구체적으로) —
      장부의 미완료 항목을 가리키고, **장부에 없는 경계 정보만** 여기 새로 쓴다
- [ ] 유예된 결정: 장부의 유예 절 참조 (복제 금지 — 정본은 장부)
