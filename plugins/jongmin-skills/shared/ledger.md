...
**파일 가치 기준 — 3목적지 분류** (실측: 한 세션이 repo 밖 4파일 생성, 그중 1개는 응답으로
냈어야 했다). 산출물은 주 소비자로 목적지를 정한다:
- 지금의 사용자가 소비 → **응답 본문** (사용자가 파일을 명시 요청한 경우만 예외)
- 현재 실행만 소비 → **scratchpad** (세션과 함께 소멸)
- 세션 이후 기계·미래 세션이 소비하거나 재계산 비용이 큼(CX 분석 등) → **이 장부 경로**

**정본 단일화** — 웨이브 상태의 정본은 진행 장부 1개다. handoff·보고·다른 문서는 그것을
참조하고 복제하지 않는다 (실측: 4곳 복제 → "다음 단계"가 2회 어긋나 2회 정정).
예외: 경계 스냅샷(handoff)은 자기완결이 필요해 사본을 담는다 — 사본에는 "스냅샷 시점
사본, 불일치 시 장부 우선"을 명시해 사본이 정본을 이기지 않게 한다.
